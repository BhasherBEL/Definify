	...
	async function buttonClick() {
		if (step === 0) {
			...
			if (password.length < 8) {
				error = get(t)('register.error.invalid.passwordTooWeak');
				return;
			}
...
>
	<div class="relative p-6 sm:p-8">
		...
		<div class="space-y-4 md:space-y-6">
			...
			{/if}
			<button
				class="w-full rounded-lg border border-transparent bg-action-zone px-5 py-2.5 text-center text-sm font-medium text-white focus:outline-none focus:ring-4 focus:ring-primary-300 hover:enabled:bg-primary-700 disabled:opacity-30 dark:bg-action-zone-dark dark:focus:ring-primary-800 dark:enabled:hover:bg-primary-700"
				onclick={buttonClick}
