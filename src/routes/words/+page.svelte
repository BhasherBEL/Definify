<script lang="ts">
	...
	let words: {
		word: string;
		definitions: { definition: string; type: string }[];
		audio: string;
	}[] = data.words.map((item) => ({
		word: item.word,
		definitions: item.definition.flatMap((d: any) =>
			d.meanings.flatMap((m: any) =>
				m.definitions.map((def: any) => ({ definition: def.definition, type: m.partOfSpeech }))
			)
		),
...
>
	{#each words as word}
		<a
			...
			class="m-2 flex items-center rounded-lg border p-4 py-2 shadow-[0_0_6px_0_rgba(0,14,156,.2)] hover:shadow-inner lg:m-0 lg:w-full lg:items-start lg:pb-0"
		>
			<div class="flex flex-grow lg:h-32 lg:flex-col">
				<div class="mr-5 flex flex-grow bg-white lg:w-full lg:flex-none">
					<div class="flex-grow text-xl capitalize">{word.word}</div>
